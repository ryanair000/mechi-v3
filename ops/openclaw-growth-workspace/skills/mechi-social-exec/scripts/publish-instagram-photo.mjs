...
async function main() {
  ...
  if (!parsed.media) {
    console.error(usageFor('publish-instagram-photo.mjs', 'image'));
    process.exit(1);
  }
  ...
  const staged = await stagePublicMediaUrl(parsed.media, brandConfig, 'image');
  ...
  containerBody.set('image_url', staged.publicUrl);
