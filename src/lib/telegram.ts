...
export async function sendOnlineTournamentCheckInTelegramNotification(params: {
  eventTitle: string;
  username: string;
  ...
  deviceSerialLast6?: string | null;
  tournamentLobbyNumber?: number | null;
  tournamentLobbySlot?: number | null;
  checkedInAt?: string | null;
  registrationId?: string | null;
}): Promise<void> {
  const gameConfig = ONLINE_TOURNAMENT_GAME_BY_KEY[params.game];
  const adminUrl = `${ADMIN_URL}/admin/online-tournament`;
  const checkInUrl = `${APP_URL}${ONLINE_TOURNAMENT_CHECK_IN_PATH}?game=${encodeURIComponent(params.game)}`;
  const lobbyLabel = formatOnlineTournamentLobby({
    game: params.game,
    tournament_lobby_number: params.tournamentLobbyNumber ?? null,
    tournament_lobby_slot: params.tournamentLobbySlot ?? null,
  });
  const message = [
    '<b>PlayMechi tournament check-in</b>',
    '',
    formatField('Event', params.eventTitle),
    formatField('Player', params.username),
    formatField('Game', gameConfig.label),
    formatField('IGN', params.inGameUsername),
    ...
    '',
    formatTelegramLink('Open tournament admin', adminUrl),
    formatTelegramLink('Open player check-in', checkInUrl),
  ]
    .filter((line): line is string => Boolean(line))
    .join('\n');
