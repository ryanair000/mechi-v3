...
export function V5TournamentDirectory({ tournaments }: { tournaments: PublicTournament[] }) {
  ...
  return <>
    <section className={styles.hero}><div className={styles.container}><div><p>Competition directory</p><h1>Find a tournament worth playing.</h1><span>Compare game, entry, schedule, region and availability before you join.</span></div><Link href="/app/organizer/tournaments/new">Host a tournament <ArrowRight size={16}/></Link><div className={styles.heroMetrics}><div><strong>{tournaments.length}</strong><span>Open public tournaments</span></div><div><strong>{free}</strong><span>Free-entry events</span></div><div><strong>Solo + team</strong><span>Competition formats</span></div><div><strong>Secure</strong><span>Paid entry support</span></div></div></div></section>
    ...
    </div><div className={styles.resultBar}><span><strong>{visible.length}</strong> matching tournaments</span><small>Only tournaments currently available to the public appear here.</small></div>
    {visible.length?<div className={styles.grid}>{visible.map((item,index)=><TournamentCard tournament={item} index={index} key={item.slug}/>)}</div>:tournaments.length===0?<div className={styles.empty}><Trophy/><h2>No open tournaments right now</h2><p>New competitions will appear here when organizers publish them.</p><Link href="/app/organizer/tournaments/new">Host the next tournament <ArrowRight size={15}/></Link></div>:<div className={styles.empty}><Trophy/><h2>No tournaments match these filters</h2><p>Try another game, entry type or search term.</p>{hasFilters?<button type="button" onClick={()=>{setQuery('');setGame('all');setEntry('all')}}>Clear filters</button>:null}</div>}</section>
    <section className={styles.hostBand}><div className={styles.container}><ShieldCheck/><div><h2>Want to host?</h2><p>Create a competition, set the rules and entry details, then publish it or submit it for approval where required.</p></div><Link href="/app/organizer/tournaments/new">Create tournament <ArrowRight size={16}/></Link></div></section>
  </>;
}
