...
const PLAYMECHI_REGISTER_PATTERNS = [
  ...
  /\bwant to (register|join|sign ?up|enter)\b/i,
  /\bneed to (register|join|sign ?up|enter)\b/i,
  /\bwould like to (register|join|sign ?up|enter)\b/i,
...
const PLAYMECHI_STREAM_PATTERNS = [
  /\bstream\b/i,
  /\byoutube\b/i,
  /\binstagram\b/i,
  /\btiktok\b/i,
    ...
    .map(
      (config) =>
        [
          ...
          ...(config.fourthPrize ? [`4th: ${config.fourthPrize}`] : []),
          ...(config.fifthPrize ? [`5th: ${config.fifthPrize}`] : []),
        ].join('\n')
    )
    .join('\n\n');
}
...
function formatPlayMechiStreamMessage() {
  const platformLinks = ONLINE_TOURNAMENT_STREAM_PLATFORMS.map(
    (platform) => `${platform.label}: ${platform.href}`
  );

  return [
    'Stream: PlayMechi on Instagram, TikTok, and YouTube',
    ...platformLinks,
