ALTER TABLE public.online_tournament_registrations
  ADD COLUMN IF NOT EXISTS entry_fee_kes integer,
...
SET payment_status = CASE
  WHEN event_slug = 'mechi-club-online-gaming-tournament-2026-05' THEN 'paid'
...
ALTER TABLE public.online_tournament_registrations
  DROP CONSTRAINT IF EXISTS online_tournament_registrations_payment_status_check;

ALTER TABLE public.online_tournament_registrations
  ADD CONSTRAINT online_tournament_registrations_payment_status_check
  CHECK (
    payment_status IN ('pending_payment', 'paid', 'failed', 'refunded', 'manual_review')
  );

ALTER TABLE public.online_tournament_registrations
  DROP CONSTRAINT IF EXISTS online_tournament_registrations_payment_tier_check;

ALTER TABLE public.online_tournament_registrations
  ADD CONSTRAINT online_tournament_registrations_payment_tier_check
  CHECK (payment_tier IS NULL OR payment_tier IN ('early_bird', 'regular', 'late'));

ALTER TABLE public.online_tournament_registrations
  DROP CONSTRAINT IF EXISTS online_tournament_registrations_entry_fee_kes_check;

ALTER TABLE public.online_tournament_registrations
  ADD CONSTRAINT online_tournament_registrations_entry_fee_kes_check
  CHECK (entry_fee_kes IS NULL OR entry_fee_kes >= 0);

CREATE INDEX IF NOT EXISTS idx_online_tournament_registrations_payment_status
  ON public.online_tournament_registrations(event_slug, payment_status);

CREATE INDEX IF NOT EXISTS idx_online_tournament_registrations_payment_tier
  ON public.online_tournament_registrations(event_slug, payment_tier)
