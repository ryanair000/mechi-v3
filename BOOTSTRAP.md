...
8. If the task touches GitHub issues, PRs, workflow runs, or repo metadata, use `skills/github-ops/SKILL.md` and `./scripts/openclaw-gh.sh`.
9. If the task touches durable notes, internal memory, handoff capture, or meeting records, use `skills/obsidian-ops/SKILL.md` and `./scripts/openclaw-obsidian.sh`.
10. If the task touches marketing or campaign ops, inspect `apps/marketing`.
11. If the task touches Android, inspect `apps/android`.
12. Run `git status --short` before edits and do not overwrite changes you do not understand.
