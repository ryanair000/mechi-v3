...
8. If the task asks about the Mechi.club Online Gaming Tournament, PlayMechi, PUBG Mobile, CODM, eFootball, event rules, prizes, schedule, stream, WhatsApp group guidance, or reward eligibility, use `skills/playmechi-tournament-ops/SKILL.md`. For live slot counts, also use `skills/supabase-live-ops/SKILL.md` and `npm run ops:registrations -- --json`.
   PlayMechi Launch / 8-10 May registration is closed. For WhatsApp registration messages like "How can I register?", "registration link", "I want to register", "register me", or "join tournament", send current/open event links instead: `https://mechi.club/weekendcup` for Weekend Cup, or `https://mechi.club/tournaments` for other open tournaments. Do not send `https://mechi.club/playmechi/register` as a registration link.
