...
export const ONLINE_TOURNAMENT_BATTLE_ROYALE_LOBBY_SIZE = 100;
export const ONLINE_TOURNAMENT_EFOOTBALL_LOBBY_SIZE = 16;
...
export function getOnlineTournamentLobbySize(game: OnlineTournamentGameKey) {
  return game === 'efootball'
    ? ONLINE_TOURNAMENT_EFOOTBALL_LOBBY_SIZE
    : ONLINE_TOURNAMENT_BATTLE_ROYALE_LOBBY_SIZE;
}

export function formatOnlineTournamentLobby(
  registration: Pick<
    OnlineTournamentRegistrationOpsRow,
    'game' | 'tournament_lobby_number' | 'tournament_lobby_slot'
  >
) {
  ...
  const shortLabel = ONLINE_TOURNAMENT_GAME_BY_KEY[registration.game].shortLabel;
  return `${shortLabel} Lobby ${registration.tournament_lobby_number} | Slot ${registration.tournament_lobby_slot}`;
}
...
export function getOnlineTournamentCheckInHref(game?: string | null) {
  const params = game ? `?game=${encodeURIComponent(game)}` : '';
  return `${ONLINE_TOURNAMENT_CHECK_IN_PATH}${params}`;
}
