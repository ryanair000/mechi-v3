...
CREATE TABLE IF NOT EXISTS weekend_cup_ballots (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS weekend_cup_ballot_options (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS weekend_cup_ballot_votes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ballot_id uuid NOT NULL REFERENCES weekend_cup_ballots(id) ON DELETE CASCADE,
  ballot_option_id uuid NOT NULL REFERENCES weekend_cup_ballot_options(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT timezone('utc', now()),
...
CREATE INDEX IF NOT EXISTS idx_weekend_cup_ballots_order
  ON weekend_cup_ballots(cup_order, created_at);
CREATE INDEX IF NOT EXISTS idx_weekend_cup_ballot_options_ballot
  ON weekend_cup_ballot_options(ballot_id, created_at);
CREATE INDEX IF NOT EXISTS idx_weekend_cup_ballot_votes_ballot
  ON weekend_cup_ballot_votes(ballot_id, created_at);
CREATE INDEX IF NOT EXISTS idx_weekend_cup_ballot_votes_user
  ON weekend_cup_ballot_votes(user_id, created_at DESC);

INSERT INTO weekend_cup_ballots (
  slug,
  title,
  subtitle,
  date_label,
...
INSERT INTO weekend_cup_ballot_options (
  ballot_id,
  slug,
  label,
  platform,
...
VALUES
  ((SELECT id FROM weekend_cup_ballots WHERE slug = 'weekend-cup-1-mobile'), 'efootball', 'eFootball', 'mobile', '1v1 bracket pressure with clean Sunday finals energy.', true),
...
ALTER TABLE weekend_cup_ballots ENABLE ROW LEVEL SECURITY;
ALTER TABLE weekend_cup_ballot_options ENABLE ROW LEVEL SECURITY;
ALTER TABLE weekend_cup_ballot_votes ENABLE ROW LEVEL SECURITY;
GRANT ALL ON weekend_cup_ballots, weekend_cup_ballot_options, weekend_cup_ballot_votes TO service_role;
REVOKE ALL ON weekend_cup_ballots, weekend_cup_ballot_options, weekend_cup_ballot_votes FROM anon, authenticated;
...
CREATE TABLE IF NOT EXISTS online_tournament_registrations (
  ...
  device_model text,
  device_serial_last6 text CHECK (device_serial_last6 IS NULL OR device_serial_last6 ~ '^[A-Z0-9]{6}$'),
  tournament_lobby_number integer CHECK (tournament_lobby_number IS NULL OR tournament_lobby_number > 0),
  tournament_lobby_slot integer CHECK (tournament_lobby_slot IS NULL OR tournament_lobby_slot > 0),
  tournament_lobby_assigned_at timestamptz,
  email text,
  ...
  entry_fee_kes integer CHECK (entry_fee_kes IS NULL OR entry_fee_kes >= 0),
  payment_tier text
    CHECK (payment_tier IS NULL OR payment_tier IN ('early_bird', 'regular', 'late')),
...
CREATE INDEX IF NOT EXISTS idx_online_tournament_registrations_payment_status
  ON online_tournament_registrations(event_slug, payment_status);
CREATE INDEX IF NOT EXISTS idx_online_tournament_registrations_payment_tier
  ON online_tournament_registrations(event_slug, payment_tier)
  WHERE payment_tier IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_online_tournament_checked_in_lobby_slot
  ON online_tournament_registrations(event_slug, game, tournament_lobby_number, tournament_lobby_slot)
...
CREATE INDEX IF NOT EXISTS idx_online_tournament_checked_in_lobby
  ON online_tournament_registrations(event_slug, game, check_in_status, tournament_lobby_number, tournament_lobby_slot);
...
CREATE TABLE IF NOT EXISTS online_tournament_result_submissions (
  ...
  screenshot_url text,
  screenshot_public_id text,
  ocr_status text CHECK (ocr_status IS NULL OR ocr_status IN ('pending', 'complete', 'failed')),
  ocr_text text,
  ocr_confidence numeric,
  ocr_kills integer CHECK (ocr_kills IS NULL OR ocr_kills >= 0),
  ocr_placement integer CHECK (ocr_placement IS NULL OR ocr_placement > 0),
  ocr_error text,
