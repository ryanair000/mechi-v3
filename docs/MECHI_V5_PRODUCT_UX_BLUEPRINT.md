...
| 42 | Full inbox and conversation | Approved | Add report/block, attachment, system-message, and closed-room states. |
| 43 | Account access and recovery | Refine | Expand sign-in, sign-up, verify, forgot/reset, MFA, lockout, and contextual return variants. |
| 44 | Role onboarding and gamer identity | Refine | Keep gamer path; add role-specific onboarding pages below. |
| 45 | Settings, privacy, security, game accounts | Approved | Add active sessions, passkey/2FA recovery, data export/delete states. |
| 46 | Support, reporting, blocking and appeals | Approved | Add case conversation, evidence upload, and resolution satisfaction states. |
| 47 | Public organizer and creator profile | Approved | Add verification and inactive-organization states. |
| 48 | Public team profile | Approved | Add recruiting/privacy/disbanded states. |
| 49 | Player wallet, receipts, prizes and payouts | Refine | Clarify ledger versus withdrawable balance; add KYC and payout-failure states. |
| 50 | Result submission and evidence timeline | Approved | Add file validation, opponent response, and immutable-source explanation. |
| 51 | Registration recovery states | Approved | Use for pending, interrupted, success, failed, expired, duplicate, and refunded payment outcomes. |
| 52 | Partner embeds, API keys and webhooks | Refine | Add permission, secret reveal/rotation, retry, rate-limit, and audit states. |
| 53 | Empty, loading, error and permission states | Approved | Extend as the system-state master matrix. |
| 54 | Player Dashboard V2 | Reference | Imported responsive-code reference; normalize into page 37 rather than creating a competing shell. |
| 55 | Creator Studio V2 | Reference | Imported responsive-code reference; merge useful content into pages 34 and the new organizer/creator flows. |

## 9. Missing Figma design scope

New pages begin after page 55. Each screen page must contain desktop 1440, tablet 1024, and mobile 390 frames unless it is explicitly a component/state documentation page.

### Batch A — component gaps

| Planned page | Contents |
| --- | --- |
| 56 Components — Identity, Roles and Workspaces | Auth field groups, password rules, OTP, MFA, account status, role card, workspace trigger/menu, step progress, verification card. |
| 57 Components — Teams, People and Invitations | Person row, roster slot, captain/starter/substitute role, invite status, permission row, readiness gate, team selector. |
| 58 Components — Data Review, Admin and Audit | Review queue row, risk badge, evidence viewer, comparison panel, decision card, audit timeline, money authorization panel. |
| 59 Components — Overlays, Confirmation and Destructive Actions | Dialog, drawer, bottom sheet, toast, command palette, undo, destructive confirmation, step-up authentication, session-expiry warning. |

### Batch B — public discovery gaps

| Planned page | Route family | Required desktop/tablet/mobile content |
| --- | --- | --- |
| 60 Screen — Games Directory and Game Hub | `/games`, `/games/[slug]` | Featured games, supported formats, live/upcoming tournaments, rankings, organizers, streams, game-specific empty state. |
