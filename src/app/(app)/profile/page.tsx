    ...
    <div className="page-container">
      ...
      <div className="card p-5 mb-5">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/20 flex items-center justify-center text-2xl font-black text-emerald-400 flex-shrink-0">
            {user?.username?.[0]?.toUpperCase() ?? '?'}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-black text-white truncate">{user?.username}</h1>
            <p className="text-xs text-white/30 mt-0.5">{profile?.region as string} · {(profile?.platforms as string[])?.length ?? 0} platforms</p>
      ...
      <div className="flex gap-1 p-1 bg-white/5 rounded-2xl mb-5">
        {(['stats', 'settings'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'stats' ? <BarChart2 size={15} /> : <Settings size={15} />}
        ...
        <div className="space-y-3">
          {userGames.length === 0 ? (
            <div className="text-center py-10 text-white/25">
              ...
              <button onClick={() => setTab('settings')} className="text-emerald-400 text-sm font-bold mt-2">
                Go to Settings →
              </button>
            </div>
          ) : userGames.map((g) => {
            ...
            return (
              <div key={g} className="card p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="font-bold text-white text-sm">{GAMES[g].label}</p>
                  <div className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: tier.color + '20', color: tier.color }}>
                    {tier.name}
                  </div>
                </div>
                <div className="grid grid-cols-4 gap-3 text-center">
                  <div><div className="text-lg font-black text-white">{rating}</div><div className="text-[10px] text-white/30 uppercase tracking-wide">ELO</div></div>
                  <div><div className="text-lg font-black text-emerald-400">{wins}</div><div className="text-[10px] text-white/30 uppercase tracking-wide">Wins</div></div>
                  <div><div className="text-lg font-black text-red-400">{losses}</div><div className="text-[10px] text-white/30 uppercase tracking-wide">Losses</div></div>
                  <div><div className="text-lg font-black text-blue-400">{wr}%</div><div className="text-[10px] text-white/30 uppercase tracking-wide">W/R</div></div>
                </div>
                {wins + losses > 0 && (
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="space-y-2">
              {(Object.keys(PLATFORMS) as PlatformKey[]).map((key) => {
                ...
                return (
                  <div key={key} className={`border rounded-2xl overflow-hidden transition-colors ${isSel ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-white/8 bg-white/3'}`}>
                    <button type="button" onClick={() => togglePlatform(key)}
                      className="w-full flex items-center gap-3 p-3.5 text-left">
                      <span className="text-xl">{plat.icon}</span>
                      <span className="font-semibold text-white flex-1 text-sm">{plat.label}</span>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSel ? 'border-emerald-500 bg-emerald-500' : 'border-white/20'}`}>
                    ...
                    {isSel && (
                      <div className="px-3.5 pb-3.5">
                        ...
                        <p className="text-xs text-white/25 mt-1">{plat.idLabel}</p>
                      </div>
                    )}
                  </div>
                );
          ...
          {selectedGamesForPlatforms.length > 0 && (
            <div>
                ...
                {selectedGamesForPlatforms.map((g) => {
                  ...
                  return (
                    <button key={g} type="button" onClick={() => toggleGame(g)}
                      className={`w-full flex items-center gap-3 p-3.5 rounded-2xl border text-left transition-all ${isSel ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-white/8 bg-white/3 hover:bg-white/5'}`}>
                      <div className="flex gap-1">
                        {GAMES[g].platforms.map((p) => <span key={p} className="text-sm">{PLATFORMS[p]?.icon}</span>)}
                      </div>
                      <span className="font-semibold text-white flex-1 text-sm">{GAMES[g].label}</span>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${isSel ? 'border-emerald-500 bg-emerald-500' : 'border-white/20'}`}>
                        {isSel && <Check size={11} className="text-white" />}
                      </div>
                    </button>
                  );
              ...
              <p className="text-xs text-white/25 mt-2 text-center">{selectedGames.length}/3 games selected</p>
            </div>
          )}
