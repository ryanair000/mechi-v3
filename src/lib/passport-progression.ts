...
export async function setPassportReplayPublic(userId: string, replayId: string, isPublic: boolean) { const { data } = await createServiceClient().from('passport_replay_snapshots').update({ is_public: isPublic }).eq('id', replayId).eq('user_id', userId).select('id').maybeSingle(); return Boolean(data); }
export async function getPublicPassportReplay(token: string): Promise<PassportReplay | null> { const supabase = createServiceClient(); const { data } = await supabase.from('passport_replay_snapshots').select('id, share_token, user_id, replay_year, period_state, payload, source_cutoff_at, is_public, generated_at, profile:profiles(username, avatar_url)').eq('share_token', token).eq('is_public', true).maybeSingle(); if (!data) return null; const profile = relation(data.profile as { username: string; avatar_url: string | null } | Array<{ username: string; avatar_url: string | null }> | null); if (!profile) return null; const { data: passport } = await supabase.from('passport_profiles').select('display_name').eq('user_id', data.user_id).maybeSingle(); return { id: String(data.id), share_token: String(data.share_token), user_id: String(data.user_id), username: profile.username, display_name: passport?.display_name ?? profile.username, avatar_url: profile.avatar_url, replay_year: Number(data.replay_year), period_state: data.period_state, payload: data.payload, source_cutoff_at: String(data.source_cutoff_at), is_public: true, generated_at: String(data.generated_at) }; }
