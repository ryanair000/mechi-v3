...
async function main() {
  ...
  if (!parsed.brand || !parsed.media) {
    console.error(usageFor('publish-meta-photo.mjs', 'image'));
    process.exit(1);
  }

  const args = toChildArgs(parsed);
  const instagram = runChild('publish-instagram-photo.mjs', args);
  const facebook = runChild('publish-facebook-photo.mjs', args);
    ...
    JSON.stringify(
      {
        ...
        error: ok ? null : instagram.error || facebook.error || 'Meta photo cross-post failed.',
      },
      null,
      2
    )
