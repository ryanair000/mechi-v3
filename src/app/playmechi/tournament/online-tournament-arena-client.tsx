...
type OnlineTournamentArenaClientProps = {
  surface?: 'player' | 'moderator';
  view?: 'check-in' | 'tournament';
};
    ...
    <div className="page-container py-8">
      <section className="card circuit-panel p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href={getRegisterPath({ next: nextPath })}
            className="btn-ghost"
          >
            Create account
          </Link>
...
export function OnlineTournamentArenaClient({
  surface = 'player',
  view,
}: OnlineTournamentArenaClientProps = {}) {
  ...
  const checkInPath =
    surface === 'moderator' ? MODERATOR_TOURNAMENT_CHECK_IN_PATH : ONLINE_TOURNAMENT_CHECK_IN_PATH;
  const arenaPath =
    surface === 'moderator' ? MODERATOR_TOURNAMENT_ARENA_PATH : ONLINE_TOURNAMENT_ARENA_PATH;
  const checkInHref = `${checkInPath}?game=${activeGameParam}`;
  const arenaHref = `${arenaPath}?game=${activeGameParam}`;
  const registrationHref = `${ONLINE_TOURNAMENT_REGISTRATION_PATH}?game=${activeGameParam}&next=${encodeURIComponent(checkInHref)}`;
  const isCheckInRoute = view === 'check-in' || pathname === checkInPath;
  const authNextPath = isCheckInRoute ? checkInHref : arenaHref;
  ...
  const [checkInForm, setCheckInForm] = useState<PlayerCheckInDetails>({
    ign: '',
    uid: '',
    device: '',
    whatsappNumber: '',
  ...
  useEffect(() => {
    if (
      ...
      !isCheckInRoute ||
      myRegistration ||
      redirectedToRegistrationRef.current
    ) {
      return;
    }

    redirectedToRegistrationRef.current = true;
    toast('Register for this tournament first, then complete check-in.');
    router.replace(registrationHref);
  }, [isCheckInRoute, myRegistration, registrationHref, router, state, user]);
      ...
      <section className="border-b border-[var(--border-color)] pb-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href={checkInHref} className="btn-ghost">
  ...
  return (
    <section className="card p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        {!registration ? (
          <Link href={registrationHref} className="btn-primary justify-center">
            Register
          </Link>
        ) : null}
      </div>
      ...
      {isDisqualified ? (
        <div className="mt-5 rounded-[var(--radius-card)] border border-red-400/20 bg-red-500/10 px-4 py-4 text-sm text-red-200">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <label className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  onCheckInDetailsChange({ ...checkInDetails, ign: event.target.value })
                }
                className="input mt-1"
              />
            </label>
            <label className="block">
              <span className="text-xs font-bold text-[var(--text-soft)]">UID</span>
              <input
                required
                value={checkInDetails.uid}
                onChange={(event) =>
                  onCheckInDetailsChange({ ...checkInDetails, uid: event.target.value })
                }
                className="input mt-1"
              />
            </label>
            ...
            <label className="block">
              <span className="text-xs font-bold text-[var(--text-soft)]">WhatsApp number</span>
              <input
                required
                type="tel"
                autoComplete="tel"
    ...
    <section className="card p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 text-xl font-black text-[var(--text-primary)]">{rules.heading}</h2>
        </div>
            ...
            <ul className="mt-3 space-y-2">
              {section.items.map((item) => (
                <li key={item} className="flex gap-2 text-sm leading-6 text-[var(--text-secondary)]">
                  <span className="mt-2 size-1.5 shrink-0 rounded-full bg-[var(--accent-secondary-text)]" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
