...
const FROM_ADDRESS = process.env.AWS_SES_FROM_EMAIL?.trim() || 'noreply@mechi.club';
...
function getAwsSesSessionToken() {
  return process.env.AWS_SES_SESSION_TOKEN?.trim() || process.env.AWS_SESSION_TOKEN?.trim() || '';
}

function getAwsSesEndpoint() {
...
function getAwsSesCredentials() {
  const accessKeyId = getAwsSesAccessKeyId();
  const secretAccessKey = getAwsSesSecretAccessKey();
  ...
  return {
    accessKeyId,
    secretAccessKey,
    sessionToken: getAwsSesSessionToken() || undefined,
  };
}

function hasAwsCredentialSource() {
  return Boolean(
...
function getAmazonSesClient() {
  const credentials = getAwsSesCredentials();
  const endpoint = getAwsSesEndpoint();
  const region = getAwsSesRegion();

  if (
    cachedSesClient &&
    cachedSesClient.region === region &&
    cachedSesClient.endpoint === endpoint &&
    cachedSesClient.accessKeyId === credentials?.accessKeyId
  ) {
    return cachedSesClient.client;
  }
...
export function isTransactionalEmailReady(): boolean {
  ...
  return Boolean(FROM_ADDRESS && hasAwsCredentialSource());
}
...
async function sendEmailWithAwsSes(payload: EmailPayload): Promise<{ messageId: string | null }> {
  ...
  try {
    const response = await getAmazonSesClient().send(command);
