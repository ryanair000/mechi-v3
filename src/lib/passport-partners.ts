...
export async function submitPassportPartnerIssuance(rawKey: string, idempotencyKey: string, input: Record<string, unknown>) {
  ...
  const username = String(input.subject_username ?? '').trim().toLowerCase().replace(/^@/, ''); const { data: subject } = await createServiceClient().from('passport_profiles').select('user_id').eq('public_handle', username).eq('publication_status', 'published').maybeSingle(); if (!subject) return { request: null, status: 404, error: 'Subject Gamer Passport not found' };
