...
export function V5TournamentDirectory({ tournaments }: { tournaments: PublicTournament[] }) {
  ...
  const hasFilters=Boolean(query.trim())||game!=='all'||entry!=='all';
  return <>
    <section className={styles.hero}><div className={styles.container}><div><p>Competition directory</p><h1>Find a tournament worth playing.</h1><span>Compare the game, format, entry, schedule and region before you join.</span></div><Link href="/app/organizer/tournaments/new">Host a tournament <ArrowRight size={16}/></Link><div className={styles.heroMetrics}><div><strong>{tournaments.length}</strong><span>Open public tournaments</span></div><div><strong>{free}</strong><span>Free, no-reward events</span></div><div><strong>Solo + team</strong><span>Competition formats</span></div><div><strong>Secure</strong><span>Protected paid entry</span></div></div></div></section>
    ...
    </div><div className={styles.resultBar}><span><strong>{visible.length}</strong> matching tournaments</span><small>Only tournaments available for public entry appear here.</small></div>
    {visible.length?<div className={styles.grid}>{visible.map((item,index)=><TournamentCard tournament={item} index={index} key={item.slug}/>)}</div>:<div className={styles.empty}><Trophy/><h2>{tournaments.length===0?'No open tournaments right now':hasFilters?'No tournaments match these filters':'No tournaments available'}</h2><p>{tournaments.length===0?'New competitions will appear here when organizers publish them.':hasFilters?'Clear a filter or try another game.':'Check back when new competitions are published.'}</p>{hasFilters?<button type="button" onClick={()=>{setQuery('');setGame('all');setEntry('all')}}>Clear filters</button>:null}</div>}</section>
    <section className={styles.hostBand}><div className={styles.container}><ShieldCheck/><div><h2>Want to host?</h2><p>Create a tournament for your community. Free no-reward events can publish after readiness checks, while paid or rewarded events require Mechi approval.</p></div><Link href="/app/organizer/tournaments/new">Create tournament <ArrowRight size={16}/></Link></div></section>
  </>;
}
...
function TournamentCard({tournament,index}:{tournament:PublicTournament;index:number}){
  ...
  return <article className={styles.card}><div className={`${styles.art} ${art}`}><span>{tournament.status==='active'?'Live':tournament.status==='open'?'Registration open':tournament.status}</span><Gamepad2/><strong>{tournament.game_label}</strong></div><div className={styles.cardBody}><div className={styles.cardTop}><div><p>{tournament.entry_fee===0?'Free entry':`KES ${tournament.entry_fee.toLocaleString('en-KE')} entry`}</p><h2>{tournament.title}</h2></div>{tournament.entry_fee>0||tournament.prize_pool>0?<span className={styles.approved}><ShieldCheck size={13}/> Entry approved</span>:<span className={styles.instant}><ShieldCheck size={13}/> No-reward event</span>}</div><div className={styles.meta}><span><CalendarClock/> {scheduled} EAT</span><span><MapPin/> {tournament.region}</span><span><UsersRound/> {tournament.player_count}/{tournament.size} players</span><span><Trophy/> {tournament.prize_pool?`KES ${tournament.prize_pool.toLocaleString('en-KE')} prize`:'No cash prize'}</span></div><div className={styles.progress}><div><span>{fill}% filled</span><small>{tournament.slots_left} slots left</small></div><i><b style={{width:`${fill}%`}}/></i></div><div className={styles.cardFooter}><span>Hosted by <strong>{tournament.organizer?.username||'Mechi organizer'}</strong></span><Link href={`/tournaments/${encodeURIComponent(tournament.slug)}`}>View details <ArrowRight size={15}/></Link></div></div></article>;
}
