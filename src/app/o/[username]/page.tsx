...
async function getOrganizerPageData(username: string) {
  ...
  const { data: tournamentsRaw } = await supabase
    .from('tournaments')
    .select(
      'id, slug, title, game, platform, region, size, entry_fee, prize_pool_mode, prize_pool, status, scheduled_for, started_at, created_at, is_featured, winner:winner_id(id, username)'
    )
